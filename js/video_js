	...
	function rlc_videos () {
		...
		function buildVideo ($vidContainer, customattributes) {
			...
			if ($vidContainer.classList.contains('rlc-loopvid')) {
				let html = '<video ' + customattributes + ' poster="' + vidPoster + '"><source src="' + vidSrc + '" type="video/mp4" />Your browser does not support the video tag.</video><div class="rlc-looppause" aria-label="play/pause button"></div>';
		...
		function addLoopVideoHandlers ($vidContainer, $theVid) {
			...
			if ($parentBlock) {
				...
				if ($parentBlock.classList.contains('rlc-isvisible') && !$vidContainer.classList.contains('user_paused')) {
					$theVid.play();
			...
			function addInViewListeners () {
				$parentBlock.addEventListener("ENTER_VIEWPORT", function() {
				...
				$parentBlock.addEventListener("LEAVE_VIEWPORT", function() {
					...
					if ($vidContainer.classList.contains("rlc-click2play")) {
						...
						if ($vidContainer.classList.contains('rlc-video-mb')) {
							...
							$videoBtnPlay.addEventListener('click', function(event) {
								...
								if (!$videoElem) { // IF VIDEO HAS NOT BEEN BUILT YET, BUILD IT
									...
									// iOS hack
									$videoElem.addEventListener('load', function (e) {
										// console.log('load');
										$videoElem.play(); // force video to load so it will show before expanding
									}, false);
							...
							$videoBtnPlay.addEventListener('click', function(event) {
								...
								if (!$videoElem) { // If video does not exist...
									$videoElem = buildVideo($vidContainer, customattributes);
									$videoElem.onplay = function() {
				...
				if (!$el.closest('.rlc-carousel') || $el.closest('.rlc-carousel--ready')) {
					...
					if(!$el.classList.contains('.rlc-lazyLoad') && !$el.closest('.rlc-lazyLoad') ) {
						...
						if ($el.classList.contains('.rlc-block')) {
							$el.addEventListener("ENTER_VIEWPORT", function() {
								rlActivateVids($el);
							}, {once:true});
						} else if ($el.closest('.rlc-block')) {
							$el.closest('.rlc-block').addEventListener("ENTER_VIEWPORT", function() {
								rlActivateVids($el);
							}, {once:true});
						}
					}
			...
			$carousels.forEach(function($carousel) {
				...
				if ($carousel.querySelectorAll('.rlc-hasvideo').length > 0) {
					if (!$carousel.classList.contains('rlc-carousel--ready')) {
						...
						$carousel.addEventListener("CAROUSEL_READY", function() {
