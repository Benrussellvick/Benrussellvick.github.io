...
// TODO: Add check to prevent from firing twice if duplicate inclusion
function RL_Carousels () {
	...
	function addADA ( $carousel ) {
		if ($carousel.classList.contains("rlc-twovis") || $carousel.classList.contains("rlc-blp-nav")) {
	...
	function updateControlADA ( $activeSlide, $carousel ) {
		...
		if ($pagination) {
			...
			for (var i = 0; i < $pagination.children.length; i++) { // KILL IE NOW!!
    ...
    function isElementVisible($el) {
        ...
        if (parentBlock) {
          ...
          if(parentBlock.classList.contains("rlc-isvisible") && rect.left >= parentRect.left && rect.left < parentRect.right) {
            return true;
          }
        }
        // Return true if any of its four corners are visible
        // return (
        else if ($el.contains(efp(rect.left,  rect.top))
